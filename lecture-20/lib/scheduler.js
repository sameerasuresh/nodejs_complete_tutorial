...
exports.recurrence = ({year= 0, month= 0, date= 0, hour= 0, minute= 0, sec= 0, miliSec= 0}, callBack) => {
    sec *= 1000;
    minute *= (1000 * 60);
    hour *= (1000 * 60 * 60);
    date *= (1000 * 60 * 60 * 24);
    ...
    let timeout;
    return {
        start:() => {timeout = setInterval(callBack, time)},
        stop: () => {
...
const sleep = ({year= 0, month= 0, date= 0, hour= 0, minute= 0, sec= 0, miliSec= 0}) => {
    clearInterval(timeout);
    sec *= 1000;
    minute *= (1000 * 60);
    hour *= (1000 * 60 * 60);
